...
<xsl:template match="pokemon">
    <xsl:value-of select="./@generation/text()" /> (<xsl:value-of select="./@pokedexNumber" />): <xsl:value-of select="./@classification" /> | 
</xsl:template> -->

<!-- <xsl:template match="/pokedex">
    <xsl:apply-templates select="pokemon[starts-with(@generation, '1')]" />
</xsl:template>
